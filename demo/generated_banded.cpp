...
int main()
{
    ...
    // std::string query_string = "ACGTTTTT";
    // std::string reference_string = "AAAAA";
    ...
    S.print_shape();
    {
        for (int k = 0; k <= K; ++k)
        {
            for (int m = std::max((k - 4) / 2, std::max(0, k - M)); m <= std::min((k + 4) / 2, std::min(N, k)); ++m)
            {
                if (((k == 0) && (m == 0)))
                {
                    S(0, 0) = 0;
...
// clang++ -std=c++17 -o out demo/generated_banded.cpp