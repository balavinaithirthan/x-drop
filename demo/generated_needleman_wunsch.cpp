...
int main()
{
    ...
    int j = 0;
    {
        for (int i = 0; i < query_string.size() + 1; ++i)
        {
            for (int j = 0; j < reference_string.size() + 1; ++j)
            {
                ...
                S(i, j) = variadic_max((S((i - 1), (j - 1)) + sigma(query((i - 1)), reference((j - 1)))), (S((i - 1), j) + -1), (S(i, (j - 1)) + -1));
            }
        }
    ...
    aligned_print("aligned query",
                  std::string(query_aligned.begin(), query_aligned.end()),
                  "aligned reference",
                  std::string(ref_aligned.begin(), ref_aligned.end()));
}
